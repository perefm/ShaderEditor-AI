...
// branching on the C++ side (spec 008, User Story 5).
...
// Toon (cel) shading (spec 008, User Story 5): quantizes the diffuse term into a fixed number of
// discrete lighting bands instead of a smooth gradient, for a stylized/cartoon look. Reuses the
// same material/bone/single-light data as assets/shaders/mega_material.glsl.
...
// Number of discrete lighting bands (e.g. 4 = classic flat-shaded cel look; higher values
// approach smooth shading).
uniform int toonBands = 4;
...
void main() {
    ...
    // Quantize into "toonBands" discrete steps instead of a continuous gradient.
    float bands = max(float(toonBands), 1.0);
    float quantized = floor(diffuseTerm * bands) / bands;

    // A thin dark rim where the surface faces away from the camera reinforces the cel-shaded
    // silhouette look, evaluated the same way rim_lighting_material.glsl computes its highlight.
    vec3 viewDir = normalize(uCameraPos - vWorldPos);
    float edge = 1.0 - max(dot(normal, viewDir), 0.0);
    float outline = edge > 0.85 ? 0.0 : 1.0;

    vec3 color = (Mat_Ka * ambientStrength + albedo * quantized * lightColor) * outline;
